...
def update_rover(rover, data):
      ...
      if rover.statistics.start_time == None:
      ...
      rover.perception.pos = [convert_to_float(pos.strip()) for pos in data["position"].split(';')]
      ...
      rover.statistics.samples_collected = rover.statistics.samples_to_find - np.int(data["sample_count"])

      print('speed =',rover.perception.vel, 'position =', rover.perception.pos, 'throttle =',
      rover.control.throttle, 'steer_angle =', rover.control.steer, 'near_sample:', rover.perception.near_sample,
      'picking_up:', data["picking_up"], 'sending pickup:', rover.control.send_pickup,
      'total time:', rover.statistics.total_time, 'samples remaining:', data["sample_count"],
...
def create_output_images(rover):
      ...
      if np.max(rover.map.worldmap[:,:,2]) > 0:
            ...
            navigable = rover.map.worldmap[:,:,2] * (255 / np.mean(rover.map.worldmap[nav_pix, 2]))
      else: 
            navigable = rover.map.worldmap[:,:,2]
      if np.max(rover.map.worldmap[:,:,0]) > 0:
            obs_pix = rover.map.worldmap[:,:,0] > 0
            obstacle = rover.map.worldmap[:,:,0] * (255 / np.mean(rover.map.worldmap[obs_pix, 0]))
      else:
      ...
      good_nav_pix = np.float(len(((plotmap[:,:,2] > 0) & (rover.map.ground_truth[:,:,1] > 0)).nonzero()[0]))
      # Next find how many do not correspond to ground truth pixels
      bad_nav_pix = np.float(len(((plotmap[:,:,2] > 0) & (rover.map.ground_truth[:,:,1] == 0)).nonzero()[0]))
      ...
      cv2.putText(map_add,"Time: "+str(np.round(rover.statistics.total_time, 1))+' s', (0, 10),
      ...
      cv2.putText(map_add,"  Located: "+str(samples_located), (0, 70), 
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      cv2.putText(map_add,"  Collected: "+str(rover.statistics.samples_collected), (0, 85),
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
